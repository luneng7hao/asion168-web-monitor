...
class Monitor {
    ...
    initErrorMonitor() {
        ...
        window.addEventListener('error', (event) => {
            var _a;
            if (!this.initialized || Math.random() > (this.config.sampleRate || 1))
                return;
            // 先检查是否是资源加载错误
    ...
    initBehaviorMonitor() {
        ...
        document.addEventListener('click', (event) => {
            ...
            this.trackBehavior('click', {
                ...
                text: ((_a = target.textContent) === null || _a === void 0 ? void 0 : _a.substring(0, 100)) || '',
                position: {
    ...
    trackBehavior(type, data) {
        const behaviorData = {
            ...
            data: Object.assign(Object.assign({}, data), { 
                // 添加更多上下文信息
                userAgent: navigator.userAgent, screenSize: {
            ...
            while (this.queue.length > 0) {
                ...
                try {
                    ...
                    if (!sent) {
                        ...
                        yield fetch(`${this.config.apiUrl}${item.endpoint}`, {
                            method: 'POST',
                            headers: {
    ...
    captureError(error, context) {
        const errorData = Object.assign({ type: 'js', message: error.message, stack: error.stack, url: this.decodeUrl(window.location.href), timestamp: new Date().toISOString(), userAgent: navigator.userAgent, userId: this.config.userId, sessionId: this.sessionId, projectId: this.config.projectId }, context);
        this.reportError(errorData);
    }
    // 手动追踪行为
    track(event, data) {
        // 自定义事件使用 'custom' 类型，但保留原始事件名在data中
        this.trackBehavior('custom', Object.assign({ eventName: event }, data));
    }
}
// 自动初始化（如果配置了）
